...
describe('AStarFinder', () => {
	...
	it('finds a straight path in an empty 3x3 board', () => {
		...
		const result = finder.findPath(board, from, to, 3) as {
			success: boolean;
			openList: any[];
			closedList: any[];
			finalPath?: any[];
		...
		expect(result.finalPath && result.finalPath.some((p: any) => p.x === 0 && p.y === 0)).toBe(
			true
		);
		expect(result.finalPath && result.finalPath.some((p: any) => p.x === 2 && p.y === 0)).toBe(
			true
		);
	});
